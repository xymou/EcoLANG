...
import random
import subprocess
from evaluation_prl import eval_dataset, cal_token_chat, cal_token_hisim, cal_token_oasis, semantic_sim_hisim
import sqlite3
...
def twitter_simulation(args):
    ...
    def process_task(config_file, args):
        ...
        if not os.path.exists(output_path):
            # simulation        
            cmd = f"""python twitter_simulation_hisim.py --config_path {config_file}"""
            print(cmd)
...
def hisim_simulation(args):
    ...
    with ThreadPoolExecutor(max_workers=args.task_workers) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(configs), desc="simulating"):
            ...
            all_token_count[scene_id] = token_count
            logger.info(f'Task {scene_id} completed!')
    # for config_file in tqdm(configs, desc="simulating"):
    #     scene_id, res, token_count = process_task(config_file,args)
    #     all_res[scene_id] = res    
    #     all_token_count[scene_id] = token_count
    #     logger.info(f'Task {scene_id} completed!')
    return all_res, all_token_count            
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default="hisim", help='dataset for simulation')
    parser.add_argument('--data_path', type=str, default="../data/hisim/", help='dataset for simulation')
    parser.add_argument('--sample_size', type=int, default=1, help='number of samples for each scenario')
    parser.add_argument('--simu_config_path', type=str, default="../configs/hisim_oasis.json", help='path of overall simulation cofig')
    parser.add_argument('--config_output_dir', type=str, default="../scen_configs/hisim_oasis/", help='dir of generated cofigs')
    parser.add_argument('--output_dir', type=str, default="../output/hisim_oasis/", help='dir of simulation output')
    ...
    parser.add_argument('--task_workers', type=int, default=1, help='num of parallel workers for simulation')
    parser.add_argument('--tokenizer', type=str, default="/remote-home/share/LLM_CKPT/huggingface_models/Llama-3.1-8B-Instruct")
    parser.add_argument('--score_ckpt_dir', type=str, default="score_ckpt/hisim_oasis/")
    ...
    simu_token_count = cal_token_hisim(simu_data, tokenizer)
    # evaluation
